...
app.delete("/employees/:id", (req, res) => {
    ...
    // First, update employees who have this employee as their manager
    const updateManagerQuery = "UPDATE employees SET manager_id = NULL WHERE manager_id = ?";
    db.query(updateManagerQuery, [id], (err, updateData) => {
        if (err) {
        ...
        db.query(deleteAttendanceQuery, [id], (err, attendanceData) => {
            ...
            // Delete leave records for the employee
            const deleteLeavesQuery = "DELETE FROM leaves WHERE employee_id = ?";
            db.query(deleteLeavesQuery, [id], (err, leavesData) => {
                if (err) {
                    console.error('Error deleting leave records:', err);
                    return res.status(500).json(err);
                }
                
                // Delete performance reviews for the employee (as both employee and reviewer)
                const deletePerformanceReviewsQuery = "DELETE FROM performance_reviews WHERE employee_id = ? OR reviewer_id = ?";
                db.query(deletePerformanceReviewsQuery, [id, id], (err, reviewsData) => {
                    if (err) {
                        console.error('Error deleting performance reviews:', err);
                        return res.status(500).json(err);
                    }
                    
                    // Delete salary records for the employee
                    const deleteSalariesQuery = "DELETE FROM salaries WHERE employee_id = ?";
                    db.query(deleteSalariesQuery, [id], (err, salariesData) => {
                        if (err) {
                            console.error('Error deleting salary records:', err);
                            return res.status(500).json(err);
                        }
